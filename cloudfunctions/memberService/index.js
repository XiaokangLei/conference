  ...
  try {
    ...
    if (memberInfos.data.length === 0) {
      let task1 = db.collection('mini_member').add({
        data: {
          ...
          modifyTime: new Date(Date.now() + (28800000)).getTime(),
    ...
    let task3 = db.collection('mini_sign_detail').add({
      data: {
        ...
        createTime: new Date(Date.now() + (28800000)).getTime()
      }
    })
    tasks.push(task3)
    ...
    let task5 = db.collection('mini_point_detail').add({
      data: {
        ...
        count: pointCount,
        desc: "签到得积分",
        date: (new Date(Date.now() + (28800000))).toFormat("YYYY-MM-DD HH24:MI:SS"),
        createTime: new Date(Date.now() + (28800000)).getTime()
      }
    })
    tasks.push(task5)
...
async function addSign(event) {
  ...
  try {
    ...
    const tasks = []
    let pointCount = 1
    let date = new Date(Date.now() + (28800000)).toFormat("YYYY-MM-DD")
    if (memberInfos.data.length === 0) {
      let task1 = db.collection('mini_member').add({
        data: {
          openId: event.info.openId,
      ...
      let memberInfo = memberInfos.data[0]
      if (new Date(Date.now() + (28800000)).addDays(-1).toFormat("YYYY-MM-DD") == memberInfo.lastSignedDate) {
    ...
    //签到明细
    let date1 = new Date(Date.now() + (28800000)).toFormat("YYYY-M-D").split("-")
    let task3 = db.collection('mini_sign_detail').add({
      data: {
        openId: event.info.openId,
        year: date1[0],
    ...
    if (event.info.accept == 'accept') {
      let task4 = await db.collection("mini_subcribute").add({
        data: {
          ...
          openId: event.info.openId,
          timestamp: new Date(Date.now() + (28800000)).getTime()
        }
      });
      tasks.push(task4)
    }
    ...
    try {
      ...
      let task3 = db.collection('mini_share_detail').add({
        data: {
          ...
          avatarUrl: event.info.avatarUrl,//头像
          nickName: event.info.nickName,//昵称
          date: (new Date(Date.now() + (28800000))).toFormat("YYYY-MM-DD"),
          createTime: new Date(Date.now() + (28800000)).getTime()
        }
      })
      tasks.push(task3)
